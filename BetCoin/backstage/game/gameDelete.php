<?php 
    ...
    if(isset($_GET["no"])){
        $no=$_GET["no"];
        $SQLGame="Select * from game where no='$no'";
        $resultGame=mysqli_query($LinkToSQL,$SQLGame);
        $rowGame=mysqli_fetch_assoc($resultGame);
        if($rowGame["outcome"]=="未知"){
            $SQLDelete="DELETE FROM game WHERE no='$no'";
            $result=mysqli_query($LinkToSQL,$SQLDelete);
            echo "<script type='text/javascript'>";
            echo "alert('刪除成功');";
            echo "</script>";
            echo "<meta http-equiv='Refresh' content='0;url=gameDelete.php'>";
        }else{
            echo "<script type='text/javascript'>";
            echo "alert('刪除失敗，比賽結果已確定');";
            echo "</script>";
            echo "<meta http-equiv='Refresh' content='0;url=gameDelete.php'>";
        }
    }
        ...
        賽局管理刪除</br>
            ...
            if(isset($_POST["search"])){
                ...
                if($searchResult){
                    ...
                    <form action="gameDelete.php" method="post" enctype="multipart/form-data">
                    ...
                    <tr>
                       <th>no</th><th>隊伍1</th><th>隊伍2</th><th>隊伍1賠率</th><th>隊伍2賠率</th><th>隊伍1勝負</th><th>頻道參數</th><th>賽局開始時間</th><th>刪除</th>
                    </tr>    
                    <?php
                    while($row=mysqli_fetch_assoc($searchResult)){
                        ...
                        echo "<tr>";
                        echo "<td>".$row["no"]."</td><td>".$row["t1No"]."-".$team1Name["teamName"]."</td><td>".$row["t2No"]."-".$team2Name["teamName"]."</td>
                            <td>".$row["team1Odds"]."</td><td>".$row["team2Odds"]."</td><td>".$row["outcome"]."</td><td>".$row["channelParameter"]."</td><td>".$row["gameStartTime"]."</td>";

                        ?>
                        <td>             
                            <a href="<?php echo "gameDelete.php?no=".$row["no"]?>">
