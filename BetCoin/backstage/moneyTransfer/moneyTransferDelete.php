<?php 
    ...
    if(isset($_GET["no"])){
        $no=$_GET["no"];
        $SQLMoneyTransfer="SELECT * FROM moneytransfer WHERE no='$no'";
        $resultMoneyTransfer=mysqli_query($LinkToSQL,$SQLMoneyTransfer);
        $moneyTransfer=mysqli_fetch_assoc($resultMoneyTransfer);


        $SQLDeleteMoney="DELETE FROM money WHERE uNo=".$moneyTransfer["uNo"]." && sourceFrom='T' && changeTime='".$moneyTransfer["comfirmDate"]."'";
        mysqli_query($LinkToSQL,$SQLDeleteMoney);
        ...
        匯款管理刪除</br>
            ...
            if(isset($_POST["search"])){
                ...
                if($searchResult){
                    ...
                    <form action="moneyTransferDelete.php" method="post" enctype="multipart/form-data">
                    ...
                    <tr>
                        <th>no</th><th>玩家編號</th><th>數量</th><th>I存入/O取出</th><th>認證</th><th colspan="2">詢問時間</th><th>認證時間</th><th>刪除</th>
                    </tr>    
                    <?php
                    while($row=mysqli_fetch_assoc($searchResult)){
                        ...
                        <td>".$row["inOrOut"]."</td><td>".$row["comfirm"]."</td>
                        <td colspan='2'>".$row["askDate"]."</td>
                        <td>".$comfirmDateCheck."</td>";
                        ?>
                        <td>             
                            <a href="<?php echo "moneyTransferDelete.php?no=".$row["no"]?>">
                ...
                <table border="3px" style="border-collapse:collapse;">
                    ...
                    <tr>
                    <th>no</th><th>玩家編號</th><th>數量</th><th>I存入/O取出</th><th>認證</th><th colspan="2">詢問時間</th></th><th>認證時間</th><th>刪除</th>
                    </tr>
                </form>
                    <?php
