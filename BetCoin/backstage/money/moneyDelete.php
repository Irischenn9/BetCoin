<?php 
    ...
    if(isset($_GET["no"])){
        $no=$_GET["no"];
        $SQLMoney="SELECT * FROM money WHERE no='$no'";
        $resultMoney=mysqli_query($LinkToSQL,$SQLMoney);
        $money=mysqli_fetch_assoc($resultMoney);
        if($money["sourceFrom"]=="T"){
            $SQLDeleteMoneyTransfer="DELETE FROM moneytransfer WHERE uNo=".$money["uNo"]." && comfirmDate='".$money["changeTime"]."'";
            mysqli_query($LinkToSQL,$SQLDeleteMoneyTransfer);
        }
        
        $SQLDelete="DELETE FROM money WHERE no='$no'";
        $result=mysqli_query($LinkToSQL,$SQLDelete);
            if($result){
                echo "<script type='text/javascript'>";
                echo "alert('刪除成功');";
                echo "</script>";
                echo "<meta http-equiv='Refresh' content='0;url=moneyDelete.php'>";
            }else{
                echo "<script type='text/javascript'>";
                echo "alert('刪除失敗');";
                echo "</script>";
        ...
        金錢管理刪除</br>
            ...
            if(isset($_POST["search"])){
                ...
                if($searchResult){
                    ...
                    <form action="moneyDelete.php" method="post" enctype="multipart/form-data">
                    ...
                    <tr>
                        <th>no</th><th>玩家編號</th><th>來源</th><th>數量</th><th>發生時間</th><th>刪除</th>
                    </tr>    
                    <?php
                    while($row=mysqli_fetch_assoc($searchResult)){
                        ...
                        echo "<tr>";
                        echo "<td>".$row["no"]."</td><td>".$row["uNo"]."-".$user["email"]."</td><td>".$row["sourceFrom"]."</td><td>".$row["changeAmount"]."</td><td>".$row["changeTime"]."</td>";
                        ?>
                        <td>             
                            <a href="<?php echo "moneyDelete.php?no=".$row["no"]?>">
